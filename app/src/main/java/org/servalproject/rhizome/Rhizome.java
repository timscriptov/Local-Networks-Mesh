...
public class Rhizome {
        ...
        try {
            ...
            if (enable != alreadyEnabled) {
                if (enable) {
                    ...
                    if (ServalBatPhoneApplication.getContext().meshMS != null)
                        ServalBatPhoneApplication.getContext().meshMS.initialiseNotification();
                } else {
                    ServalDCommand.configActions(
            ...
            public int message(String cmd, Iterator<String> args, InputStream in, int dataBytes) throws IOException {
                ...
                if (cmd.equalsIgnoreCase("BUNDLE")) {
                    try {
                        ...
                        ServalBatPhoneApplication.getContext().runOnBackgroundThread(new ExamineBundle(manifest));
                    } catch (Exception e) {
    ...
    private static class ExamineBundle implements Runnable {
            ...
            try {
                if (manifest instanceof RhizomeManifest_MeshMS) {
                    ...
                    if (identity != null && identity.sid.equals(meshms.getRecipient()))
                        if (ServalBatPhoneApplication.getContext().meshMS != null)
                            ServalBatPhoneApplication.getContext().meshMS.bundleArrived(meshms);
                    ...
                    if (file.getFilesize() != 0) {
                        ...
                        ServalBatPhoneApplication.getContext().sendBroadcast(
                                mBroadcastIntent,
                                RECEIVE_PERMISSION);
        ...
        private void testUpgrade(RhizomeManifest_File file) {
            try {
                ServalBatPhoneApplication app = ServalBatPhoneApplication.getContext();
