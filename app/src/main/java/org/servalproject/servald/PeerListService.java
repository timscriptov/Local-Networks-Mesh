...
public class PeerListService {
    ...
    private static boolean checkContacts(Peer p) {
        ContentResolver resolver = ServalBatPhoneApplication.getContext().getContentResolver();
    ...
    public static void resolve(final Peer p) {
        ...
        if (ServalBatPhoneApplication.getContext().isMainThread()) {
            ServalBatPhoneApplication.getContext().runOnBackgroundThread(new Runnable() {
                @Override
                public void run() {
                    resolve(p);
                }
        ...
        try {
            if (lookupSocket == null) {
                lookupSocket = ServalBatPhoneApplication.getContext().server.getMdpDnaLookup(
